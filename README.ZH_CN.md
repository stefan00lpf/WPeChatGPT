...
- 当前 *WPeChatGPT* 支持的**功能**包括：
   - 分析函数的使用环境、预期目的、函数功能。
   - **增强的函数分析**：结合伪代码、完整汇编代码、交叉引用、字符串和元数据进行综合分析。
   - **交叉引用分析**：自动识别函数的调用者和被调用函数，帮助理解函数在程序中的位置。
   - **字符串提取**：自动提取函数中引用的所有字符串，辅助分析函数行为。
   - **函数元数据**：显示函数大小、类型（导入/导出/内部）、所在段、基本块数量等信息。
   - 重命名函数的变量。
...
|2.7|2026-03-11|1. **增强函数分析功能**：新增交叉引用（xrefs）分析，显示函数的调用者和被调用函数。<br>2. **字符串提取**：自动提取并显示函数中引用的所有字符串。<br>3. **函数元数据**：显示函数大小、类型、段信息、基本块数量等详细信息。<br>4. **完整汇编分析**：分析时同时提供伪代码和完整汇编代码。<br>5. **LM Studio支持**：默认配置支持本地LM Studio模型。|
## 安装
1. 运行如下命令安装所需包。
```
