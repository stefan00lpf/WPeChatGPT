...
|2.7|2025-03-11|1. **增强函数分析功能**：新增交叉引用（xrefs）分析，显示函数的调用者和被调用函数。<br>2. **字符串提取**：自动提取并显示函数中引用的所有字符串。<br>3. **函数元数据**：显示函数大小、类型、段信息、基本块数量等详细信息。<br>4. **完整汇编分析**：分析时同时提供伪代码和完整汇编代码。<br>5. **LM Studio支持**：默认配置支持本地LM Studio模型。|
## 安装
1. 运行如下命令安装所需包。
```
